...
plot_raincloud <- function(data,y,horizontal=FALSE,groups,...) {
  ...
  if(is.null(groups)) {
    p <- ggplot2::ggplot(data, ggplot2::aes(x="", y=!!y, ...)) + 
      ...
      ggplot2::geom_point(position = position_jitternudge(jitter.width = .25, nudge.x = -.175), size = .25, alpha = .25) +
    ...
    p <- ggplot2::ggplot(data, ggplot2::aes(x=!!groups,y=!!y, fill = !!groups, colour = !!groups)) +
      geom_flat_violin(position=ggplot2::position_nudge(x=.05,y=0), adjust =2, trim = TRUE) +
      ggplot2::geom_point(position = position_jitternudge(jitter.width = .25, nudge.x = -.175), size = .25) +
      ggplot2::geom_boxplot(outlier.shape = NA, alpha = 0.3, width = .1, colour = "BLACK") +
